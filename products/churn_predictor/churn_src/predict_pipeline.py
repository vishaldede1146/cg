...
class ChurnPredictor:
    ...
    def predict(self, months: list, explain: bool = True, threshold: float = 0.5) -> dict:
        ...
        X_scaled = self._prepare_sequence(months)

        prob = float(self.model.predict(X_scaled, verbose=0).ravel()[0])
        ...
        if explain:
            explanation = explain_instance(self.model, self.background, X_scaled, self.feature_cols)
            result["top_factors"] = [
